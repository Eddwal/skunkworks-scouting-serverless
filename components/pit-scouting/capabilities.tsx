...
export function CapabilityRow({ label, name, control, hasCan = true, hasAuto = true }: { label: string, name: string, control: any, hasCan?: boolean, hasAuto?: boolean }) {
  return (
...
export function SectionHeader({ title, hasCan = true, hasAuto = true }: { title: string, hasCan?: boolean, hasAuto?: boolean }) {
  return (
    <div className="grid grid-cols-[1fr_4rem_4rem] gap-2 items-center pt-2 pb-1 border-b-2 border-primary/20 px-3">
      <h4 className="font-semibold text-primary">{title}</h4>
      <span className="text-xs font-semibold text-muted-foreground uppercase text-center w-full">{hasCan ? 'Can' : ''}</span>
...
export const CapabilityViewerRow = ({ label, can, auto, hasCan = true, hasAuto = true }: { label: string, can?: boolean, auto?: boolean, hasCan?: boolean, hasAuto?: boolean }) => (
  <div className="flex items-center justify-between py-2 border-b last:border-0 border-border/50 px-1">
    ...
    <div className="flex items-center space-x-4 w-32 justify-end">
      <div className="flex justify-center w-12">
        {hasCan && can !== undefined && <YesNoBadge val={can} />}
      </div>
      {hasAuto && (
        <div className="flex justify-center w-12">
